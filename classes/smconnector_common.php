...
abstract class EmoSMConnectorCore
{
    ...
    abstract public function getSMConnectorOptions();
...
//    const SESSION_NAME                       = 'emagicone_smconnector';
...
//        $this->image_file = $shop_cart_overrider::IMAGE_FILE;
        ...
        $this->getErrors();
        $this->smconnector_options = $this->shop_cart->getSMConnectorOptions();
        $this->checkSMConnectorOptions();
        if (1 === $this->smconnector_options['use_plugin_tmp_dir']) {
            $this->tmp_folder_path = emo_smc_get_plugin_tmp_path();
        } else {
            $this->tmp_folder_path = $this->shop_cart->getShopRootDir().$this->smconnector_options['tmp_dir'];
        }
        $timestamp = time();
        ...
        if (!isset($this->smconnector_options['last_clear_date'])
            ...
            $this->shop_cart->saveConfigData($this->smconnector_options);
        }
        ...
        if ($this->shop_cart->issetRequestParam('task')) {
            $this->smConnectorAction();
        } else {
            $this->deleteSessionKey();
            die(
                $this->shop_cart->jsonEncode(
                    array(
    ...
    private function checkSMConnectorOptions()
    {
    ...
    private function smConnectorAction()
    {
    ...
    private function checkDataBeforeRun()
    {
        ...
        if (ini_get('open_basedir') && $this->smconnector_options['tmp_dir'] == null) {
    ...
    private function getDbDump()
    {
        ...
        $this->downloadDump($this->dump_file_current, $this->smconnector_options['allow_compression']);
    }
        ...
        foreach ($this->db_tables as $table) {
            ...
            while ($result && $count_result > 0) {
                ...
                if ($this->getGeneratingDumpValue(self::GET_SQL_CANCEL_PARAM)) {
                    ...
                    if ($this->shop_cart->fileExists($path_em1_smconnector_db_dump_sql)) {
                        $this->dbFileClose();
    ...
    private function generateArchive()
    {
        if ($this->smconnector_options['allow_compression']) {
            ...
            $fp_gz = $this->shop_cart->gzFileOpen($fname_gz_path, "wb{$this->smconnector_options['compress_level']}");
            ...
            if ($fp_gz && $fp) {
                while (!feof($fp)) {
                    $content = $this->shop_cart->fileRead($fp, $this->smconnector_options['package_size']);
...
//        if ($this->backup_file_ext == 'gz') {
    ...
    private function getDbFile()
    {
        ...
        while (($read_size > 0) && ($package_size > 0)) {
            ...
            echo $this->shop_cart->fileRead($fp, $read_size);
    ...
    private function putSqlRun($data)
    {
        ...
        $this->shop_cart->execSql("SET SQL_BIG_SELECTS=1;");
    ...
    {
        echo "0\r\n";
        echo 'database_host='.esc_textarea($this->shop_cart->getDbHost())."<br>\r\n";
        echo 'database_name='.esc_textarea($this->shop_cart->getDbName())."<br>\r\n";
        echo 'database_username='.esc_textarea($this->shop_cart->getDbUsername())."<br>\r\n";
        echo 'database_password='.esc_textarea($this->shop_cart->getDbPassword())."<br>\r\n";
        echo 'database_table_prefix='.esc_textarea($this->shop_cart->getDbPrefix())."<br>\r\n";
        echo 'php_version='.phpversion()."<br>\r\n";
        echo 'gzip='.(int)extension_loaded('zlib')."<br>\r\n";

        if (defined('VM_VERSION')) {
            echo 'vm_version='.esc_textarea(VM_VERSION)."<br>\r\n";
        }
    }
    ...
    private function getCategoryTree()
    {
        ...
        die($this->generateFileData($this->tmp_folder_path.'/'.$tmpfname, $this->smconnector_options['allow_compression']));
    }
    ...
    private function getVars()
    {
        ...
        echo '1|'.wp_kses($this->shop_cart->jsonEncode($translations), 'entities');
    }
    ...
    private function getItemsList($path_xml_file, $fields, $items_node, $items_info_node, $filters)
    {
        ...
        echo '1|'.wp_kses($this->shop_cart->jsonEncode($items_list), 'entities')."\r\n";
    ...
    {
        echo esc_textarea($this->shop_cart->getCartVersion());
    }
    ...
    private function checkDataChanges()
    {
        ...
        echo wp_kses($this->shop_cart->checkDataChanges(explode(';', call_user_func('base64_decode', $table_name))), 'entities');
    }
    ...
    private function getNewOrders()
    {
        ...
        echo wp_kses($this->shop_cart->getNewOrders($order_id), 'entities');
    }
    ...
    private function setImage()
    {
        ...
        if ($this->shop_cart->issetRequestParam('image_url')) {
            ...
            if (sanitize_textarea_field($_FILES[self::UPLOAD_FILE_NAME]['error'])) {
...
//        }
    ...
    private function getFilesDetails()
    {
        ...
        if ($filterCounter == count($details)) {
            self::generateResponse($filesDetails);
        }
        ...
        foreach ($files as $file) {
            ...
            $filePath = (string)$this->shop_cart->getFile('', (string)$file['path']);
            $fileInformation['path'] = (string)$file['path'];
            $fileExists = ($filePath && $this->shop_cart->fileExists($filePath));
            if (array_key_exists('size', $details) && (boolean)$details['size'] && $fileExists) {
                $fileInformation['size'] = (int)$this->shop_cart->fileSize($filePath);
            } elseif (array_key_exists('size', $details) && (boolean)$details['size']) {
                $fileInformation['size'] = 0;
            }

            if (array_key_exists('type', $details) && (boolean)$details['type'] && $fileExists) {
                $fileInformation['type'] = (string)@filetype($filePath);
            } elseif (array_key_exists('type', $details) && (boolean)$details['type']) {
                $fileInformation['type'] = '';
            }

            if (array_key_exists('permissions', $details) && (boolean)$details['permissions'] && $fileExists) {
                $fileInformation['permissions'] = (int)fileperms($filePath);
            } elseif (array_key_exists('permissions', $details) && (boolean)$details['permissions']) {
                $fileInformation['permissions'] = 0;
            }

            $filesDetails[] = $fileInformation;
        }
    ...
    {
        echo wp_kses($this->shop_cart->getCache(), 'entities');
    }
     ...
     */
    public static function generateResponse($data = array(), $code = 20)
    {
    ...
    {
        $jsonInput = json_decode($this->shop_cart->fileGetContents('php://input'), true);
        if (!is_array($jsonInput) && $this->shop_cart->issetRequestParam('data')) {
            $jsonInput = json_decode((string)$this->shop_cart->getRequestParam('data'), true);
        }
    ...
    private function generateError($err_text = '1', $error_code = false, $show_link = false)
    {
        if ($show_link) {
            ...
            echo wp_kses($this->shop_cart->jsonEncode(
                array(
...
//            echo $err_text;
    ...
    private function addFailedAttempt()
    {
        ...
        $sql = 'INSERT INTO `'.self::TABLE_FAILED_LOGIN
            ."` (`ip`, `date_added`) VALUES ('".sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR']))."', '"
        ...
        $sql = 'SELECT COUNT(`id`) AS count_rows FROM `'.self::TABLE_FAILED_LOGIN
            ."` WHERE `ip` = '".$this->shop_cart->pSQL(sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'])))."' AND `date_added` > '"
